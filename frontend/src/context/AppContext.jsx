...
function mapRemoteDocument(row) {
  return {
    ...
    uploaderUserId: row.uploader_user_id || null,
    ...
    approvedSignatureId: row.approved_signature_id || null,
          ...
          if (remoteEnabled) {
            ...
            nextDocuments = chainDocs.map((item) => {
              ...
              return {
                ...
                organizationId: existing?.organizationId || item.organizationId || null,
                uploaderUserId: existing?.uploaderUserId || item.uploaderUserId || userId,
                subjectUserId: existing?.subjectUserId || item.subjectUserId || userId,
                workflowStatus: existing?.workflowStatus || item.workflowStatus || "pending",
                rejectionReason: existing?.rejectionReason || item.rejectionReason || "",
                submittedAt: existing?.submittedAt || item.submittedAt || null,
                reviewedAt: existing?.reviewedAt || item.reviewedAt || null,
                reviewedBy: existing?.reviewedBy || item.reviewedBy || null,
                approvedSignatureId: existing?.approvedSignatureId || item.approvedSignatureId || null,
                currentVersion: Number(existing?.currentVersion || item.currentVersion || 1),
                verificationHash: existing?.verificationHash || item.verificationHash || "",
